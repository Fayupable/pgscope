...
// a "column = ?"-shaped comparison across query texts already known to
// reference the candidate table. Same approach as the Postgres adapter's
// extractSuspectedColumns, but matching MySQL's DIGEST_TEXT placeholder
// (?) instead of Postgres's ($1, $2, ...). Best-effort and advisory —
// pattern matching over already-masked query text, never literal values.
...
func findComparedColumns(text string) []string {
	...
	for i := 0; i+2 < len(fields); i++ {
		...
		columns = append(columns, strings.ToLower(strings.Trim(fields[i], "`")))
	}
